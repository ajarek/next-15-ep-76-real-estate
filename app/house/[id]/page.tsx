 ...
 const {id} = await params
  ...
  return (
    <div className='min-h-[calc(100vh-32px*2-48px)] grid grid-cols-2 max-lg:grid-cols-1 p-4 gap-4 place-items-center'>
      ...
      <MotionImage src={hoseId?.image || ''} alt='fotografia domu' width={540} height={306}/>
      </div>
      <div>
        <h1 className='text-2xl font-bold'>{'Dom Jednorodzinny'} id-00{hoseId?.id}</h1>
        ...
        <p className='text-lg text-gray-500 mb-4'>Rok budowy: <span className='font-semibold text-foreground'>{hoseId?.yearBuilt}</span></p>
        <MotionLink href={`/offer?id=${hoseId?.id}&image=${hoseId?.image}`} label='Jestem zainteresowany'/>
      </div>
    </div>
  )
}
