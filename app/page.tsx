...
export default async function Home({ searchParams}: { searchParams: Promise<{ city: string, price: string, bedrooms: string , squareFeet: string, yearBuilt: string  }>
}) {
  ...
  return (
    <main className='min-h-[calc(100vh-32px*2-48px)] grid grid-cols-[3fr_1fr] max-lg:grid-cols-1 p-4 gap-4'>
      ...
      <Hause houses={houses} city={city} price={price} bedrooms={bedrooms} squareFeet={squareFeet} yearBuilt={yearBuilt} />
      </div>
      <SidebarSearch />
    </main>
  )
