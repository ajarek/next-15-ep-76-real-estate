  ...
  return (
    <>
      {houses
        .filter(
          (house: HouseTypes) =>
            !city || house.city.toUpperCase().includes(city.toUpperCase())
        )
        .filter((house: HouseTypes) => !price || house.price <= Number(price))
        .filter(
          (house: HouseTypes) =>
            !squareFeet || house.squareFeet <= Number(squareFeet)
        )
        .filter(
          (house: HouseTypes) => !bedrooms || house.bedrooms == Number(bedrooms)
        )
        .filter(
          (house: HouseTypes) =>
            !yearBuilt || house.yearBuilt == Number(yearBuilt)
        )
            ...
            >
              ...
              <CardContent className='p-2 flex flex-col items-start justify-start gap-4 '>
                ...
                <div className='w-full grid grid-cols-3 gap-2 place-items-center'>
                  ...
                  <div className='flex flex-col items-center'>
                    <Ruler color='#dc2626 ' />{' '}
                    <div className='font-semibold'>{house.squareFeet}</div>
                    <div>Pow/m²</div>
                  </div>
                </div>
              </CardContent>
